...
可以假设`org.apache.ignite.tests.pojos.Person`类的实现如下：
```java
public class Person {
...
和示例4相比可以看到，使用Kryo序列化器，`phones`字段会被序列化到`blob`类型的`phones`列。另外，Cassandra会为`married`列创建二级索引。
...
`log4j.properties`文件非常简单，它就是`log4j`的logger配置，下面会深入地了解下其它的配置文件。
