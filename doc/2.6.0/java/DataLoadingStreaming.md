...
**数据加载**
从像Ignite持久化存储或者第三方存储这样的数据源中进行数据的预加载，在下面的`5.2.数据加载`章节中有详细描述。
**工作方式**
...
下面是一个第三方存储的`CacheStore.loadCache()`实现的例子，对于`CacheStore`的完整例子，可以参照`16.2.第三方存储`章节。
```java
