...
|属性名|描述|默认值|
|---|---|---|
|`user`|SQL连接的用户名，如果服务端开启了认证则此参数为必需。关于如何开启认证和创建用户，可以分别参见[认证](/doc/2.8.0/java/Security.md#_2-1-认证)和[创建用户](/doc/2.8.0/sql/SQLReference.md#_2-6-create-user)的文档。|`ignite`|
|`password`|SQL连接的密码，如果服务端开启了认证则此参数为必需。关于如何开启认证和创建用户，可以分别参见[认证](/doc/2.8.0/java/Security.md#_2-1-认证)和[创建用户](/doc/2.8.0/sql/SQLReference.md#_2-6-create-user)的文档。|`ignite`|
...
```
### 1.5.流处理
Ignite的JDBC驱动可以通过`SET STREAMING`命令对流化数据进行批量处理，具体可以看[SET STREAMING](/doc/2.8.0/sql/SQLReference.md#_4-2-set-streaming)的相关内容。
## 2.JDBC客户端模式驱动
### 2.1.JDBC客户端模式驱动
...
```
::: tip 安全连接
关于如何保护JDBC客户端驱动的更多信息，请参见[高级安全](/doc/2.8.0/java/Security.md#_2-高级安全)的相关文档。
:::
它支持如下的参数：
...
::: tip 跨缓存查询
驱动连接到的缓存会被视为默认的模式，要跨越多个缓存进行查询，可以参照[分布式关联](/doc/2.8.0/sql/Architecture.md#_2-分布式关联)章节。
:::
...
:::
这些参数几乎覆盖了`IgniteDataStreamer`的所有常规配置，这样就可以根据需要更好地调整流处理器。关于如何配置流处理器可以参考[流处理器](/doc/2.8.0/java/DataLoadingStreaming.md#_3-数据流处理器)的相关文档来了解更多的信息。
::: tip 基于时间的刷新
