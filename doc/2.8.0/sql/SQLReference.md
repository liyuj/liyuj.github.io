...
#### 2.3.1.参数
 ...
 - `WITH`：标准ANSI SQL中未定义的参数：
   ...
   - `ATOMICITY=<ATOMIC | TRANSACTIONAL | TRANSACTIONAL_SNAPSHOT>`：为底层缓存设置[原子化模式](/doc/2.8.0/java/Key-ValueDataGrid.md#_8-1-1-原子化模式)模式，如果该参数和`TEMPLATE`参数都未指定，那么创建的缓存为`ATOMIC`模式。如果指定为`TRANSACTIONAL_SNAPSHOT`模式，那么该表会[支持事务](/doc/2.8.0/java/Key-ValueDataGrid.md#_8-3-多版本并发控制)；
   ...
   - `AFFINITY_KEY=<affinity key column name>`：设置[关联键](/doc/2.8.0/java/Key-ValueDataGrid.md#_7-关联并置)名字，它应该是`PRIMARY KEY`约束中的一个列；
...
该表将存储在连接参数中指定的模式中。如果未指定模式，将使用`PUBLIC`模式。有关Ignite中模式的详细信息，请参见[模式](/doc/2.8.0/sql/Architecture.md#_6-模式)。
...
`ASYNC`是一个可选参数，可以立即返回而无需等待取消完成，`query_id`可以通过[SQL QUERIES](/doc/2.8.0/java/Metrics.md#_4-12-sql_queries)视图进行检索。
