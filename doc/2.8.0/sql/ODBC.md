...
Ignite的ODBC驱动的源代码随着Ignite版本一起发布，在使用之前可以自行进行构建。关于如何获取和设置Ignite本身，可以参照[基本概念](/doc/2.8.0/java/)章节。
...
在实现层，Ignite的ODBC驱动使用SQL字段查询来获取Ignite缓存中的数据，这意味着通过ODBC只可以访问这些[集群配置中定义](/doc/2.8.0/sql/JavaDeveloperGuide.md#_2-模式和索引)的字段。
...
:::
### 3.9.流处理
Ignite的ODBC驱动可以通过`SET STREAMING`命令对流化数据进行批量处理，具体可以看[SET STREAMING](/doc/2.8.0/sql/SQLReference.md#_4-2-set-streaming)的相关内容。
::: tip 注意
