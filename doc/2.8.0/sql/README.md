...
### 2.2.SQL接入
作为入门来说，可以使用一个SQL工具，在后面的[SQL工具](/doc/2.8.0/sql/ToolsAndAnalytics.md#_1-sql工具)章节中会有一个示例来演示如何配置SQL工具。还可以使用[SQLLine](/doc/2.8.0/sql/ToolsAndAnalytics.md#_2-sqlline)接入集群然后在命令行执行SQL语句。
...
`CREATE TABLE`命令执行之后，会做如下的工作：
 ...
 - 会自动使用表名创建一个分布式缓存，它会用于存储City和Person类型的对象，这个缓存存储的City和Person对象可以与特定的Java、.NET和C++类或者[二进制对象](/doc/2.8.0/java/#_8-二进制编组器)相对应；
 - 会定义一个带有所有参数的SQL表；
...
在上面的示例中，对于`Person`表，Ignite创建了一个有一份备份数据的分布式缓存，`city_id`作为关联键，这些扩展参数是Ignite特有的，通过`WITH`进行传递，要为表配置其它的缓存参数，需要使用`template`参数，并且使用之前注册的缓存配置的名字（通过代码或者XML），具体可以参照[扩展参数](/doc/2.8.0/sql/SQLReference.md#_2-3-1-参数)相关章节。
