...
或者也可以使用Ignite原生的持久化-这是一个分布式的、支持ACID以及兼容SQL的磁盘存储，它可以在存储于内存和磁盘上的数据执行SQL查询。
 ...
 7. 通过为特定的Ignite缓存指定持久化配置，或者通过使用`@QuerySqlField(index = true)`注解自动进行配置的检测，支持Cassandra的[二级索引](http://docs.datastax.com/en/cql/3.0/cql/cql_reference/create_index_r.html)（包括定制索引）;
...
下面是一个Ignite将Cassandra配置为缓存存储的典型配置示例，即使它看上去很复杂也不用担心，下面会一步一步深入每一个配置项，这个示例来自于Cassandra模块源代码的单元测试资源文件`test/resources/org/apache/ignite/tests/persistence/blob/ignite-config.xml`。
...
```
在这个配置中可以看到Cassandra的`test1.blob_test1`表会用于**cache1**缓存的键/值存储，缓存的键对象会以**integer**的形式存储于`key`列中，缓存的值对象会以**blob**的形式存储于`value`列中。
...
和示例4相比，可以看到，使用Kryo序列化器，`phones`字段会被序列化到`blob`类型的`phones`列。另外，Cassandra会为`married`列创建二级索引。
...
`log4j.properties`文件非常简单，它就是`log4j`的logger配置，后续可以深入地了解下其它的配置文件。
