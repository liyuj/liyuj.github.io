...
```
#### 3.3.3.客户端事务
Java瘦客户端支持键-值事务，该Transactions API与Ignite原生客户端中的[事务](/doc/2.8.0/java/Key-ValueDataGrid.md#_8-1-2-ignitetransactions)API类似。

事务只可以在`AtomicityMode.TRANSACTIONAL`[原子化模式](/doc/2.8.0/java/Key-ValueDataGrid.md#_8-1-1-原子化模式)的缓存中执行。
...
客户端事务可以有不同的[并发模型和隔离级别](/doc/2.8.0/java/Key-ValueDataGrid.md#_8-2-并发模型和隔离级别)，和执行超时时间，这些都可以在整体/每个事务层级进行配置。
...
SQL查询共有2种：
 ...
 - **数据定义语言语句**：用来管理缓存和索引，具体可以参见[数据定义语言](/doc/2.8.0/sql/SQLReference.md#_2-数据定义语言（ddl）)；
 - **数据操作语言语句**：用来管理数据，具体请参见[数据操作语言](/doc/2.8.0/sql/SQLReference.md#_3-数据操作语言（dml）)。
...
```
### 3.5.二进制类型
瘦客户端完全支持[二进制编组器](/doc/2.8.0/java/#_8-二进制编组器)章节中描述的二进制对象API。
...
```
::: tip 确保开启服务端的认证
确保[服务端的认证](/doc/2.8.0/java/Security.md#_2-高级安全)已开启，并且在凭据存储中配置了客户端凭据。
:::
#### 3.6.3.授权
...
### 4.6.安全
#### 4.6.1.认证
关于如何在Ignite的集群端打开和配置认证，说明在[这里](/doc/2.8.0/java/Security.md#_2-高级安全)。在Node.js端，将用户名/密码传递给`IgniteClientConfiguration`的方法如下：
```javascript
...
3.设置Ignite服务端以支持[SSL和TLS](/doc/2.8.0/java/Security.md#_1-ssl和tls)，在启动过程中提供获得的`keystore.jks`和`truststore.jks`证书；
...
```
### 5.4.SQL
Python瘦客户端完整支持Ignite的SQL查询，具体的示例可以看[这里](/doc/2.8.0/sql/README.md#_2-入门)。
#### 5.4.1.准备
首先建立一个连接：
```python
client = Client()
...
Ignite服务端必须配置为保护二进制协议端口，服务端配置过程可以分为以下若干个基本步骤：
 ...
 2. 根据本章节：[保护节点之间的连接](/doc/2.8.0/java/Security.md#_1-1-保护节点间的连接)，打开Ignite集群的`SSLContextFactory`；
...
#### 5.6.2.密码认证
要进行身份验证，必须在Ignite的XML配置文件中将`authenticationEnabled`属性设置为`true`，并启用持久化，具体请参见[认证](/doc/2.8.0/java/Security.md#_2-1-认证)中的相关内容。
...
### 6.6.安全
#### 6.6.1.认证
关于如何在Ignite的集群端打开和配置认证，说明在[这里](/doc/2.8.0/java/Security.md#_2-高级安全)。在PHP端，将用户名/密码传递给`ClientConfiguration`的方法如下：
```php
const ENDPOINT = 'localhost:10800';
const USER_NAME = 'ignite';
const PASSWORD = 'ignite';
