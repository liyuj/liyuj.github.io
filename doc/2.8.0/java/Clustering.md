...
::: warning 使用服务网格
注意对于大多数`领导者`或者`类单例`场景中，建议使用`服务网格`功能，它可以自动地部署各种[集群单例服务](/doc/2.8.0/java/ServiceGrid.md#_3-集群单例)，而且更易于使用。
:::
...
如果隔离的集群使用了Ignite[持久化](/doc/2.8.0/java/Persistence.md)，那么在文件系统中每个集群应该将持久化文件保存在不同的路径中。通过`DataStorageConfiguration`中的`setStoragePath(...)`、`setWalPath(...)`、`setWalArchivePath(...)`方法可以针对每个单独的集群进行修改。
:::
#### 6.1.6.Apache JClouds IP探测器
具体请参见[JClouds发现](/doc/2.8.0/integration/Clustering.md#_3-jclouds发现)。
#### 6.1.7.Amazon S3 IP探测器
具体请参见[Amazon AWS发现](/doc/2.8.0/integration/Clustering.md#_1-amazon-aws发现)。
#### 6.1.8.Amazon ELB IP探测器
具体请参见[Amazon AWS发现](/doc/2.8.0/integration/Clustering.md#_1-amazon-aws发现)。
#### 6.1.9.Google云存储IP探测器
具体请参见[Google计算发现](/doc/2.8.0/integration/Clustering.md#_2-google计算发现)。
#### 6.1.10.JDBC探测器
...
具体请参见[Kubernetes IP探测器](/doc/2.8.0/java/KubernetesDeployment.md#_2-5-kubernetes-ip探测器)。
#### 6.1.13.ZooKeeper IP探测器
...
值得一提的是，ZooKeeper发现仅仅是发现机制的一个实现，不会影响Ignite节点间的通信（可以看[网络配置](#_9-网络配置)章节）。节点之间一旦通过ZooKeeper发现机制彼此探测到，它们就会使用Communication SPI进行点对点的通信。
#### 6.2.2.配置
...
在`CONTINUOUS`模式中，当主节点离开网格时类不会卸载。卸载只会发生于类的用户版本发生变化时。这个方式的优势是可以使来自不同主节点的任务在工作节点共享同一个用户资源的实例（参见[资源注入](/doc/2.8.0/java/#_6-资源注入)），这使得在工作节点上执行的所有任务可以复用，比如，连接池或者缓存的同一个实例。当用这个模式时，可以启动多个独立的工作节点，在主节点定义用户资源并且在工作节点上初始化一次，不管它们来自那个主节点。与`ISOLATED`部署模式相比，它在单一主节点上有一个单一类加载器的作用域，`CONTINUOUS`模式会向所有主节点扩展部署作用域，这对于生产环境非常有用。
