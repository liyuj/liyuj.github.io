...
### 1.1.激活、冻结和拓扑管理
首先，`./control.sh`用于集群基线拓扑的激活/冻结以及节点的管理，具体可以参见[基线拓扑](/doc/java/Persistence.md#_5-基线拓扑)的章节。
::: danger 警告
冻结会释放所有的内存资源，包括所有节点上的应用数据，并禁用集群的公共API。如果内存中的缓存没有通过[持久化存储](/doc/java/Persistence.md)进行备份，会丢失所有的数据，然后缓存需要重新加载。
:::
### 1.2.缓存状态监控
