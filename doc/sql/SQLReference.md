...
#### 2.2.3.1.参数
 ...
 - `WITH`：标准ANSI SQL中未定义的参数：
   ...
   - `AFFINITY_KEY=<affinity key column name>`：设置[类同键](/doc/java/Key-ValueDataGrid.md#_3-7-类同并置)名字，它应该是`PRIMARY KEY`约束中的一个列；
...
::: warning 注意
只有按主键或类同键对结果分组（即使用`GROUP BY`）时，才支持`GROUP_CONCAT`函数内的`DISTINCT`和`ORDER BY`表达式。此外，如果使用了[Java API](https://ignite.apache.org/releases/latest/javadoc/org/apache/ignite/cache/query/SqlFieldsQuery.html#setCollocated-boolean-)，必须在连接串中配置`collocated=true`或者调用`SqlFieldsQuery.setCollocated(true)`，以通知Ignite数据是并置的。
:::
