...
## 2.Kafka连接器入门
### 2.1.Kafka连接器生态系统
在一个分布式的Kafka连接器生态系统中，有不同类型的节点，本章节中使用以下的术语来指代特定的节点类型：
...
GridGain的Kafka连接器没有导入和导出使用Java注释指定的字段信息。例如，Kafka中不存在基于注解配置的关联键、SQL可查询字段和索引。
...
|源API|Ignite事件|Ignite持续查询|
|接收API|Ignite数据流处理器|Ignite数据流处理器|

## 8.示例：使用Kafka连接器在关系数据库中持久化Ignite数据
