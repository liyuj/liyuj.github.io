...
对于纯内存缓存，当分区丢失时，除非数据重新加载，否则分区中的数据将无法恢复。对于持久化缓存，数据不会物理丢失，因为它已被持久化到磁盘上。当发生故障或断开连接的节点回到集群时（重启后），将从磁盘上加载数据。这时需要重置丢失的分区的状态才能继续使用数据，具体请参见[处理分区丢失](#_3-3-处理分区丢失)章节的介绍。
...
|`READ_ONLY_SAFE`|缓存处于只读状态，缓存的写操作会抛出异常，丢失分区的读操作也会抛出异常，具体请参见[处理分区丢失](#_3-3-处理分区丢失)章节的介绍。|
