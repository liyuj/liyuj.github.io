...
**参数**
 ...
 - `index_option`：创建索引的其它选项：
   - `INLINE_SIZE`：指定索引内联大小（字节），根据这个大小，Ignite会将整个索引值或者其一部分直接放入索引页中，这样会避免对数据页的额外调用以及提高查询性能。索引内联是默认开启的，然后其大小是根据表结构预先自动计算好的。将其值配置为0可以禁用内联功能，但是通常不推荐，具体可以看[增加索引内联](/doc/java/PerformanceTroubleshooting.md#_4-8-增加索引内联值)章节的介绍；
...
**参数**
 ...
 - `origin_node_id`、`routine_id`：可以通过[CONTINUOUS_QUERIES](/doc/java/Monitoring.md#_6-15-continuous_queries)视图获得。
