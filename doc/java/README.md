...
4. `/var/log/apache-ignite`中可以看到Ignite的日志，或者使用命令：
```shell
journalctl -fe
```
如果需要在系统启动是启动节点，可以这样：
...
## 1.4.Maven配置
这里### 1.4.1.`<config_name>`参数是相对于`/etc/apache-ignite`文件夹的。
 4. `/var/log/apache-ignite`中可以看到Ignite的日志，或者使用命令：
源码包中的DEVNOTES.txt文件，有更多的细节。 5. 
 6.```shell
 ...
 >``````摘要如果项目里用Maven管理依赖，可以单独地导入各个Ignite模块，
