...
Ignite C++基于Ignite，因此首先需要构建Java代码，具体可以参见Ignite版本的[相关页面](/doc/2.7.0/java/#_1-3-入门)。
...
### 5.1.二进制编组器配置
Ignite的[二进制编组器](/doc/2.7.0/java/#_1-10-二进制编组器)负责集群中的数据、逻辑和消息的序列化和反序列化。由于架构的特殊性，Java和C++节点使用不同的二进制编组器默认配置启动，如果要建立异构集群，则可能导致节点启动过程中的异常，如下面的一个节点：
```
...
### 7.1.安装
瘦客户端API与Ignite C++一起分发，并与完整的Ignite C++ API共享许多类，头和库，因此可以轻松地从一个API切换到另一个API。基本安装和构建过程与[这里的描述](/doc/2.7.0/cpp/#_1-3-入门)是相同的。
### 7.2.要求
所有要求都与[普通客户端](/doc/2.7.0/cpp/#_1-3-1-要求)相同，但Java部分除外：不需要Java。
### 7.3.配置服务端节点
...
```
### 7.5.认证
如果服务端开启了[身份认证](/doc/2.7.0/java/Security.md#_2-1-认证)，那么必须提供用户的凭据：
