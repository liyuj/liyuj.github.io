...
```
## 3.缓存模式
Ignite C++提供了不同的缓存操作模式，可以通过Spring的XML配置文件对其进行配置，具体请参见Ignite的[分区和复制](/doc/2.7.0/java/Key-ValueDataGrid.md#_3-1-分区和复制)文档。
## 4.缓存查询
...
```
### 4.3.SQL查询
这部分内容在单独的[分布式SQL](/doc/2.7.0/cpp/DistributedSQL.md)中进行介绍。
### 4.4.文本查询
...
```
有关`ignite::Reference`类的详细信息，可以参见上面的[对象生命周期](/doc/2.7.0/cpp/#_1-6-对象生命周期)文档。
### 6.2.事件传递保证
