...
启用持久化后，就不再需要将所有数据和索引保存在内存中，也无需在节点或集群重启后进行内存预热，因为Ignite[固化内存](/doc/2.7.0/net/DurableMemory.md#_1-固化内存)与持久化是紧密耦合的，并将其视为二级存储，这意味着如果内存中缺少部分数据或索引，则固化内存将从磁盘中获取数据。

具体细节请参见Java版本的[相关文档](/doc/2.7.0/java/Persistence.md)。
