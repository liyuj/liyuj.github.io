...
```
## 3.缓存模式
Ignite.NET也提供了缓存操作的几种不同模式，具体细节可以参见Ignite的[分区和复制](/doc/2.7.0/java/Key-ValueDataGrid.md#_3-1-分区和复制)文档。
...
更多细节请参见[数据流处理器](/doc/2.7.0/net/Streaming.md#_2-数据流处理器)的文档。

### 8.2.ICache.LoadCache()
将大量数据加载到缓存的另一种方法是通过[ICacheStore.LoadCache()](/doc/2.7.0/net/Persistence.md#_2-3-1-loadcache)方法，该方法甚至无需传递需要加载的键就可以加载缓存数据。
