...
如果使用了这个方式，客户端需要在一个特定的元数据中持有模式，然后将其发送/接收给/自Ignite服务端，具体可以看[二进制类型元数据](/doc/2.7.0/java/ThinClients.md#_2-5-二进制类型元数据)的相关章节。
...
关于可用的数据类型和数据格式规范的清单，请参见[数据格式](#_2-2-数据格式)章节的介绍。
...
```
### 3.5.二进制类型
瘦客户端完全支持[二进制编组器](/doc/2.7.0/java/#_1-10-二进制编组器)章节中描述的二进制对象API。
...
```
::: tip 确保开启服务端的认证
确保[服务端的认证](/doc/2.7.0/java/Security.md#_2-高级安全)已开启，并且在凭据存储中配置了客户端凭据。
:::
#### 3.6.3.授权
...
### 4.6.安全
#### 4.6.1.认证
关于如何在Ignite的集群端打开和配置认证，说明在[这里](/doc/2.7.0/java/Security.md#_2-高级安全)。在Node.js端，将用户名/密码传递给`IgniteClientConfiguration`的方法如下：
```javascript
...
3.设置Ignite服务端以支持[SSL和TLS](/doc/2.7.0/java/Security.md#_1-ssl和tls)，在启动过程中提供获得的`keystore.jks`和`truststore.jks`证书；
...
与缓存同步、再平衡、关联和其它与缓存配置相关的详细信息，可以参见[数据网格](/doc/2.7.0/java/Key-ValueDataGrid.md)的相关文档。
...
```
### 5.4.SQL
Python瘦客户端完整支持Ignite的SQL查询，具体的示例可以看[这里](/doc/2.7.0/sql/README.md#_2-入门)。
#### 5.4.1.准备
首先建立一个连接：
```python
client = Client()
...
### 6.6.安全
#### 6.6.1.认证
关于如何在Ignite的集群端打开和配置认证，说明在[这里](/doc/2.7.0/java/Security.md#_2-高级安全)。在PHP端，将用户名/密码传递给`ClientConfiguration`的方法如下：
```php
const ENDPOINT = 'localhost:10800';
const USER_NAME = 'ignite';
const PASSWORD = 'ignite';
